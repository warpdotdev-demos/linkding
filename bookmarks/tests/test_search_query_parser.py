...
class SearchQueryTokenizerTest(TestCase):
    ...
    def test_term_with_parentheses_in_name(self):
        tokenizer = SearchQueryTokenizer("hello(world)")
        tokens = tokenizer.tokenize()
        self.assertEqual(len(tokens), 2)
        self.assertEqual(tokens[0].type, TokenType.TERM)
    ...
    def test_term_does_not_consume_unmatched_closing_parenthesis(self):
        tokenizer = SearchQueryTokenizer("(a or b)")
        tokens = tokenizer.tokenize()
        self.assertEqual(len(tokens), 6)
        self.assertEqual(tokens[0].type, TokenType.LPAREN)
        self.assertEqual(tokens[1].type, TokenType.TERM)
        ...
        # Grouping next to a bare term keeps working
        tokenizer = SearchQueryTokenizer("#a (b or c)")
        tokens = tokenizer.tokenize()
...
class SearchQueryParserTest(TestCase):
    ...
    def test_tag_with_parentheses_in_name_within_group(self):
        ...
        # Boolean grouping without parenthesized tag names stays intact
        result = parse_search_query("(#a or #b)")
        expected = _or(_tag("a"), _tag("b"))
        self.assertEqual(result, expected)

        result = parse_search_query("not (#a or #b)")
        expected = _not(_or(_tag("a"), _tag("b")))
        self.assertEqual(result, expected)

        result = parse_search_query("#a (b or c)")
        expected = _and(_tag("a"), _or(_term("b"), _term("c")))
        self.assertEqual(result, expected)
    ...
    def test_lax_term_with_parentheses_in_name(self):
        result = parse_search_query("hello(world)")
        expected = _term("hello(world)")
        self.assertEqual(result, expected)
...
class StripTagFromQueryTest(TestCase):
    ...
    def test_tag_with_parentheses_in_name(self):
        ...
        # Quoted form also strips cleanly
        result = strip_tag_from_query('#"hello(world)" and #books', "hello(world)")
        self.assertEqual(result, "#books")
